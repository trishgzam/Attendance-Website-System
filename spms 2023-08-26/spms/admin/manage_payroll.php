...
<div class="card">
    <div class="card-header">
        <h5 class="card-title"><?php echo isset($_GET['id'])? "Update" :"Create New" ?> Employee's Payroll</h5>
    </div>
    <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <select name="employee_id" id="employee_id" class="form-select form-select-sm rounded-0">
                                <option value="" disabled <?php echo !isset($payroll_id) ? "selected" : "" ?>>Select Employee Here</option>
                                <?php 
                                $qry =  $conn->query("SELECT *,CONCAT(lastname , ', ' , firstname , ' ' , COALESCE(middlename,'')) as `name` FROM `employee_list` where `status` = 1 order by CONCAT(lastname , ', ' , firstname , ' ' , COALESCE(middlename,'')) asc");
                                while($row = $qry->fetch_array()):
                                ?>
                                <option value="<?php echo $row['employee_id'] ?>" <?php echo isset($employee_id) && $employee_id == $row['employee_id'] ? "selected" : "" ?>><?php echo $row['code']. " - ". $row['name'] ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="daily_rate" class="control-label">Daily Rate</label>
                            <input type="number" step="any" name="daily_rate" id="daily_rate" class="form-control form-control-sm rounded 0 text-end" value="<?php echo isset($daily_rate) ? $daily_rate : 0 ?>">
                            <input type="hidden" step="any" name="monthly_rate" id="monthly_rate" class="form-control form-control-sm rounded 0 text-end" value="<?php echo isset($monthly_rate) ? $monthly_rate : 0 ?>">
                        </div>
                        <div class="form-group">
                            <label for="hourly" class="control-label">Rate Per Hour</label>
                            <input type="number" step="any" name="hourly" id="hourly" class="form-control form-control-sm rounded 0 text-end" value="<?php echo isset($hourly) ? $hourly : 0 ?>">
                        </div>
                        <div class="form-group">
                            <label for="per_minute" class="control-label">Rate Per Minute</label>
                            <input type="number" step="any" name="per_minute" id="per_minute" class="form-control form-control-sm rounded 0 text-end" value="<?php echo isset($per_minute) ? $per_minute : 0 ?>">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="col-md-4">
                        ...
                        <div id="earnings-holder">
                            <?php if(isset($payroll_id)): ?>
                                ...
                                while($row=$earnings->fetch_array()):
                                ?>
    ...
    <div class="col-auto">
        <button class="btn btn-primary rounded-0 me-2" form="calculator-form">Calculator</button>
        <form id="calculator-form" method="post" action="calculator.php"></form>
    </div>
    ...
    $(function(){
        ...
        $('#employee_id').change(function(){
            ...
            $.ajax({
                ...
                error:err=>{
                    console.log(err)
                    alert("There's an error occured while fetching the employee's salary rate.")
                },
                success:function(resp){
                    if(resp.status == 'success'){
