...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT p.*,CONCAT(e.lastname , ', ' , e.firstname , ' ' , COALESCE(e.middlename,'')) as `name`,e.code, d.name as dept, dd.name as desg FROM `payroll_list` p inner join employee_list e on p.employee_id = e.employee_id inner join department_list d on e.department_id = d.department_id inner join designation_list dd on e.designation_id = dd.designation_id where p.payroll_id = '{$_GET['id']}'");
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <th class="text-end bg-success bg-opacity-25 py-1" colspan="2">Net Pay</th>
                                <th class='text-end py-1'><?php echo number_format($net_pay + $total_earnings, 2) ?></th>
                            </tr>
